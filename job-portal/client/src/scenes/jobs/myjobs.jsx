...
const MyJobs = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      dispatch(setLoading());
      try {
        const response = await axios.get(import.meta.env.VITE_SERVER_URL + `/api/job/employerJobs?page=${currentPage}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
        if (response.data.success) {
          setMyJobs(response.data.jobs.data);
  ...
  return (
    <Box m={config.mainM}>
      <Header title='My Jobs' subtitle='Manage your Jobs' />
      <Box display='grid' gridTemplateColumns={`repeat(${config.resultGrid},1fr)`} gap={isNonMediumScreen ? '10px' : '5px'}>
        {myJobs?.map((job) => (
          <CustomCard key={myJobs.id} job={job} type='Job' employer={true} />
        ))}
      </Box>
      {myJobs.length === 0 && (
        <Box>
